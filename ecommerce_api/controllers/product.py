...
class Product(http.Controller):
    ...
    def get_search(self, page=None, term=None):
        ...
        if uid:
            ...
            try:
                response = json.dumps(
                    {"data": {'product': product_ids}, 'total_pages': totalpages, 'message': 'All product'})
                return Response(
                    response, status=200,
    ...
    def get_all_categories(self, page=int(1), limit=None):
        ...
        for i in category_ids:
            category_id = i['id']
            image_url = self.url + '/web/image?' + 'model=product.public.category&id=' + str(
    ...
    def get_product_by_category_id(self, category_id, page=int(1), **kw):
        ...
        if valid_token:
            ...
            product_price_list = models.execute_kw(self.db, uid, self.password, 'product.pricelist.item', 'search_read',
        ...
        try:
            ...
            if valid_token:
                products = models.execute_kw(
                    ...
                    {'fields': ['id', 'name', 'type', 'uom_name', 'cost_currency_id', 'categ_id', 'list_price',
                                'description_sale', 'accessory_product_ids', 'x_studio_specifications',
                                'x_studio_video', 'x_studio_why_and_when', 'product_template_image_ids',
                                'x_studio_product_feature_mobile', 'tax_string', 'x_studio_pdf_link', 'x_studio_breif',
                                'x_studio_why_and_when', 'x_studio_specifications', 'product_variant_ids'], 'limit': 1}
                )
                user_id = int(valid_token[0]['x_studio_user_name'][0])
                ...
                products = models.execute_kw(
                    ...
                    {'fields': ['id', 'name', 'type', 'uom_name', 'cost_currency_id', 'categ_id', 'description_sale',
                                'accessory_product_ids', 'x_studio_specifications', 'x_studio_why_and_when',
                                'x_studio_video', 'product_template_image_ids', 'x_studio_product_feature_mobile',
                                'tax_string', 'x_studio_pdf_link', 'x_studio_breif', 'x_studio_why_and_when',
                                'x_studio_specifications', 'product_variant_ids'], 'limit': 1}
                )
            ...
            for i in products:
                ...
                for alternative_ids in i['accessory_product_ids']:
                    ...
                    alternative_product = models.execute_kw(
                        ...
                        {'fields': ['id', 'product_tmpl_id', 'name', 'type', 'uom_name', 'cost_currency_id', 'categ_id',
                                    'description_sale', 'x_studio_specifications', 'x_studio_why_and_when',
                                    'product_template_image_ids', 'x_studio_product_feature_mobile', 'tax_string',
                                    'x_studio_pdf_link', 'x_studio_breif', 'x_studio_why_and_when',
                                    'x_studio_specifications', 'attribute_line_ids'], 'offset': (page - 1) * 5,
                         'limit': 5}
                    )
                    if alternative_product:
                        for j in alternative_product:
                            ...
                            image_url1 = self.url + '/web/image?' + 'model=product.template&id=' + str(
                                id) + '&field=image_1920'
                            j['image'] = image_url1
                ...
                for variant in i['product_variant_ids']:
                    ...
                    x2 = models.execute_kw(
                        self.db, uid, self.password, 'product.product', 'search_read',
                        [[['id', '=', variant]]],
                        {'fields': ['id', 'name', 'lst_price', 'valid_product_template_attribute_line_ids',
                                    'attribute_line_ids', 'product_template_variant_value_ids']})
                    # print('asd >>>>> ', x2)
                    x3 = models.execute_kw(
                        ...
                        {'fields': ['id', 'name', 'attribute_id', 'attribute_line_id', 'display_name',
                ...
                if i['product_template_image_ids']:
                    for item in i['product_template_image_ids']:
                        ...
                        if images:
                            im_url = self.url + '/web/image?' + 'model=product.image&id=' + str(
                                item) + '&field=image_1920'
                            im.append({
                                'id': images[0]['id'],
                                'image': im_url
                            })
                            images = False
                image_url = self.url + '/web/image?' + 'model=product.product&id=' + str(
                    product_id) + '&field=image_1920'
                i['image'] = image_url
                categ_id = i['categ_id'][0]
                im.append({
    ...
    def get_all_shipping_methods(self, **kw):
        ...
        shipping_id = models.execute_kw(self.db, uid, self.password, 'delivery.carrier', 'search_read',
                                        [[['id', '!=', 1]]], {
                                            'fields': ['id', 'name', 'delivery_type', 'fixed_price', 'free_over',
                                                       'amount']})
    ...
    def add_shipping_to_cart(self, shipping_id, **kw):
        ...
        if valid_token:
            ...
            if user_quot:
                ...
                try:
                    product_ship_id = models.execute_kw(self.db, uid, self.password, 'product.product', 'search_read',
                                                        [[['id', '=', shipping_id[0]['product_id'][0]]]],
                                                        {'fields': ['list_price']})
                    # print('product_ship_id', product_ship_id)
                    product_ship_temp_id = models.execute_kw(self.db, uid, self.password, 'product.product',
                                                             'search_read',
                                                             [[['id', '=', int(product_ship_id[0]['id'])]]],
                                                             {'fields': ['list_price']})
                    # print('product_ship_id', product_ship_id)
                    cart_line_id = models.execute_kw(self.db, uid, self.password, 'sale.order.line', 'create', [
                        {'product_id': shipping_id[0]['product_id'][0], 'order_id': user_quot[0]['id'],
                         'name': shipping_id[0]['name'], 'customer_lead': 2.0, 'salesman_id': '1',
                         'price_unit': product_ship_temp_id[0]['list_price'], 'product_uom_qty': 1.0,
                         'product_uom': '1'}])
                except Exception as e:
                ...
                try:
                    ...
                    if user_quot:
                        ...
                        x = models.execute_kw(self.db, uid, self.password, 'mail.message', 'create',
                                              [{'body': 'The customer has selected %s to make the payment.' % term,
                                                'model': 'sale.order',
                                                'res_id': user_quot[0]['id'],
                                                'message_type': 'comment', }])
