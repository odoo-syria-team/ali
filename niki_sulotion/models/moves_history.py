        ...
        for rec in self:
            if rec.picking_id:
                if rec.picking_id.origin.startswith("P"):
                    ...
                    pol = self.env['purchase.order.line'].search([('id' , '=' , po.id),('product_id' , '=' , rec.product_id.id)],limit=1)
                    ...
                    pol = self.env['sale.order.line'].search([('id' , '=' , po.id),('product_id' , '=' , rec.product_id.id)],limit=1)
                    rec.price = pol.price_subtotal
                else:
                    rec.price = 0
class StockPicking(models.Model):
